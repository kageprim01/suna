...
describe('agentica pgSchema', () => {
  test('declares the agentica schema namespace', () => {
    expect(kortixSchema.schemaName).toBe('agentica');
  });

  test('all sampled tables live in the agentica schema', () => {
    const tables = [accounts, projects, sandboxes, deployments, kortixApiKeys];
    for (const t of tables) {
      expect(getTableConfig(t).schema).toBe('agentica');
    }
  });
});

describe('agentica enums', () => {
...
describe('projects table', () => {
  ...
  test('manifest_path defaults to agentica.toml', () => {
    const col = getTableConfig(projects).columns.find((c) => c.name === 'manifest_path');
    expect(col?.default).toBe('agentica.toml');
  });
...
describe('kortixApiKeys table', () => {
  test('maps to the api_keys table name inside the agentica schema', () => {
    const cfg = getTableConfig(kortixApiKeys);
    expect(cfg.name).toBe('api_keys');
    expect(cfg.schema).toBe('agentica');
  });
