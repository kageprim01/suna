...
export const kortixSchema = pgSchema('agentica');
...
export const projects = kortixSchema.table(
  'projects',
  {
    ...
    manifestPath: text('manifest_path').default('agentica.toml').notNull(),
 ...
 *
 * Sources of truth:
 *   - agentica.toml `[[sandbox.templates]]` entries → upserted into this table on read
...
export const deployments = kortixSchema.table(
  'deployments',
  {
    ...
    // its agentica.toml. Populated by the /v1/projects/:id/apps path; nullable
 ...
 * Connectors are DEFINED in agentica.toml ([[connectors]]) and materialized here
 ...
 * in agentica.toml. Patterns are fully-qualified (`<slug>.<path>` globs) and apply
 ...
 * today). Materialized from [policy] in agentica.toml; missing block = allow_all
