-- Order-independent structural signature of the managed `agentica` schema:
...
--   psql "$URL" -X -At -f scripts/schema-signature.sql | sed -e 's/agentica\.//g' -e 's/public\.//g' | sort
...
FROM information_schema.columns WHERE table_schema='agentica';
SELECT 'ENUM|'||t.typname||'|'||string_agg(e.enumlabel, ',' ORDER BY e.enumsortorder)
FROM pg_type t JOIN pg_enum e ON e.enumtypid=t.oid JOIN pg_namespace n ON n.oid=t.typnamespace
WHERE n.nspname='agentica' GROUP BY t.typname;
SELECT 'IDX|'||indexname||'|'||regexp_replace(indexdef,'^CREATE','C') FROM pg_indexes WHERE schemaname='agentica';
SELECT 'CON|'||c.conrelid::regclass::text||'|'||c.conname||'|'||pg_get_constraintdef(c.oid)
FROM pg_constraint c JOIN pg_namespace n ON n.oid=c.connamespace WHERE n.nspname='agentica';
