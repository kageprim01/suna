...
<overview>
Every Agentica project has a **project brain** at `.kortix/memory/` — a
...
same content. Edits land on `main` only via an Agentica change request —
...
only via an Agentica change request — same path as any code change:
