...
Every call goes through the Agentica Executor Gateway. The gateway resolves
credentials server-side, enforces sharing and policy, executes the upstream call,
and audits the result. The sandbox carries an Agentica session/user token such as
...
Inside an Agentica sandbox, the CLI is still the fastest way to inspect the
...
code; pass only Agentica auth/context from env and let the gateway resolve
third-party secrets.
</sdk-workflows>
...
Agentica connector/secret setup command when exposed by the CLI).
</adding-connectors>
