...
- In `apps/api/src/config.ts:272-276`, missing `AGENTICA_URL` is only fatal when `ENV_MODE === 'cloud'`.
- The exact cloud-mode trigger is therefore `ENV_MODE=cloud` in the API process env.
- `AGENTICA_BILLING_INTERNAL_ENABLED` can also enable billing-related validation, but it does **not** trigger the `AGENTICA_URL Required in cloud mode` error by itself.
...
- `AGENTICA_BILLING_INTERNAL_ENABLED=true` → `KORTIX_BILLING_INTERNAL_ENABLED=false`

With `ENV_MODE=local`, `config.ts` auto-derives `AGENTICA_URL` to `http://localhost:8008/v1/router` if it is unset, so startup no longer fails on that validation.
