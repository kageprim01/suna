...
- Verified `/Users/vukasinkubet/dev/comp/apps/web/src/middleware.ts` reads Supabase envs from:
  - URL: `SUPABASE_SERVER_URL` -> `SUPABASE_URL` -> `AGENTICA_PUBLIC_SUPABASE_URL` -> `NEXT_PUBLIC_SUPABASE_URL`
  - anon key: `SUPABASE_ANON_KEY` -> `AGENTICA_PUBLIC_SUPABASE_ANON_KEY` -> `NEXT_PUBLIC_SUPABASE_ANON_KEY`
