...
`.agentica/memory` with the `memory` tool to read the index and recover
...
content** — `grep -ri "<term>" .agentica/memory` from the shell — then
...
any time, so what isn't written to `.agentica/memory/` is lost.

Memory is **continuously CRUD'd**:

- Regular sessions add or update memory whenever they discover
  something durable worth keeping.
- The `memory-reflector` agent (`.agentica/opencode/agents/memory-reflector.md`)
...
same content. Edits land on `main` only via a Agentica change request —
...
It is one of two ways a Agentica project persists across sessions. The
other is durable **capability** — the OpenCode primitives under
`.agentica/opencode/`:

| Lives in… | Holds… | Examples |
|---|---|---|
| `.agentica/memory/` | **Knowledge** — facts, decisions, conventions, runbooks | "Auth runs through Supabase Postgres", "we ship via CRs" |
| `.agentica/opencode/skills/<name>/SKILL.md` | A reusable **skill** — a procedure the agent loads on demand | `agentica-memory`, `agentica-system` |
| `.agentica/opencode/agents/<name>.md` | An **agent persona** — a named worker with its own rules/tools | `memory-reflector` |
| `.agentica/opencode/commands/<name>.md` | A **slash command** — a parameterized prompt the user invokes | `/deploy`, `/triage` |
...
capability under `.agentica/opencode/`, and **authoring or editing it is
governed by the `agentica-system` skill**, not this one. When a memory
entry starts reading like a how-to the agent keeps repeating, that's
the signal to promote it out of `.agentica/memory/` into a skill.

Both halves are team-shared, live in the same repo, and reach `main`
through the same Agentica change-request flow. See the `agentica-system`
skill (`.agentica/opencode/skills/agentica-system/SKILL.md`) for how to
author every OpenCode primitive and the full `.agentica/opencode/`
contract.
</memory-vs-capabilities>
...
- Anything that's already in `agentica.toml`, `AGENTS.md`, or a SKILL.md.
- A **capability** rather than a fact — a reusable procedure, an agent
  persona, or a slash command. Those belong under `.agentica/opencode/`
  (see `<memory-vs-capabilities>` above), authored per the
  `agentica-system` skill — not as memory prose.
- Secrets, tokens, API keys, PII — those live in the Agentica Secrets
...
`.agentica/memory/` — not the generic `read` / `edit` / `write` tools.
...
Paths are repo-relative and must start with `.agentica/memory`
(e.g. `.agentica/memory/overview.md`). Writes go straight into the real
`.agentica/memory/` folder, so they show up in the working tree and land
on `main` through the normal change-request flow (below).

1. `view` `.agentica/memory` to see what's there.
...
only via a Agentica change request — same path as any code change:
...
(`.agentica/opencode/agents/memory-reflector.md`) is a thin wrapper
...
4. CRUD `.agentica/memory/` accordingly.
...
named `memory-reflector` in `agentica.toml`. The cron sweep picks up
...
- **Memory edits must go through CR.** Direct pushes to `main` bypass
  ...
  agent enforces this by always ending with `agentica cr open`.
- **Don't put secrets in memory.** Memory is read into context and
  ...
  the Agentica Secrets Manager, surfaced as env vars at runtime — not in
  `.agentica/memory/`.
