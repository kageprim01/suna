---
name: agentica-executor
description: How to reach third-party systems from a Agentica session via the Executor — one interface to every configured integration (Pipedream, MCP, OpenAPI, GraphQL, HTTP, chat `channel`s like Slack, and connected machines via `computer`), exposed as the `agentica-executor` MCP server's tools (connectors, discover, describe, call). Load whenever the user asks the agent to DO something in an external app/API (send an email, create a Stripe charge, post to Slack, query an internal API, call any SaaS), asks "what integrations/connectors/tools do I have", asks to add/configure a connector, or asks about `[[connectors]]` in agentica.toml. The agent must use the Executor's MCP tools rather than hand-rolling API calls with raw tokens.
---

<skill name="agentica-executor">

<overview>
The **Executor** is the one way an agent reaches outside systems. Instead of
juggling per-app SDKs and raw tokens, you use the **`agentica-executor` MCP
server**, auto-loaded into every session. It talks to the Agentica **Executor
...
- **`add_connector` / `remove_connector`** — declare or remove project
  connectors through the platform (committed to `agentica.toml` + synced)
...
`$AGENTICA_EXECUTOR_TOKEN`, which makes every call act **as the user who launched
the session** — so you can only use connectors that user has been granted.

A **connector** is one named integration. They're declared in `agentica.toml` as
...
secret value / Pipedream 1-click connection is entered by the human in Agentica and
...
- Add or configure a connector, or asks about `[[connectors]]` in `agentica.toml`.

If the task is purely local (editing files, running tests) you don't need this.
</when-to-load>

<usage>
Use the `agentica-executor` MCP tools. They appear in your tool list once the
session has the Executor wired (it always does in a Agentica sandbox). All return
JSON.
...
{ "connector": "github", "action": "request", "args": {
    "method": "POST",
    "url": "https://api.github.com/repos/kageprime/suna/issues/1234/comments",
...
- To add a connector, prefer the Executor's `add_connector` tool (or
  `agentica executor add` locally); it commits the `agentica.toml` change and syncs
  the catalog. Then use `connect` / `request_secret` for credentials.
- A `agentica executor` CLI exists too (same gateway, same auth) — and the same
  Executor core is also the `@kortix/executor-sdk` TypeScript framework. The MCP
  tools are the primary path, though — prefer them.
</rules>

<adding-connectors>
Connectors are defined in `agentica.toml` (committed). Example:
...
`agentica-computer` skill). The Executor materializes the catalog after the
declaration lands. (`channel` and `computer` are SYNTH-ONLY — they appear when you
connect Slack / a machine, you don't declare them in agentica.toml. For Slack you'll
usually use the dedicated `slack` CLI — see the `agentica-slack` skill — but it's the
...
```sh
agentica executor add github --provider pipedream --app github
agentica executor connect github   # prints a one-click OAuth URL — open it, authorize
```
