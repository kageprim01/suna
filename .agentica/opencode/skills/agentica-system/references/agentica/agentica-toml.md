# `agentica.toml` — in-depth reference

`agentica.toml` is the single source of truth for everything the
Agentica platform reads about a project. It lives at the repo root.
Any repo with a valid `agentica.toml` at the root is a Agentica project.
...
# Dockerfile. Both paths default to ".agentica/Dockerfile" / "." when
# omitted — declaring them just makes the intent explicit.
[sandbox]
dockerfile = ".agentica/Dockerfile"   # repo-relative
context = "."                       # build context

# OpenCode runtime config dir. Defaults to ".agentica/opencode" when
# omitted. The agent daemon launches opencode with
# OPENCODE_CONFIG_DIR pointed here. OpenCode-native runtime config
# remains in this directory; Agentica-side launchability and grants live
...
# ─── Apps ─────────────────────────────────────────────────────────
# `[[apps]]` declares deployable surfaces alongside the agent. The
# reconcile sweep ships drift via the configured deployment provider
# (Freestyle today; pluggable).
...
enabled = true
framework = "next"
# domains is optional — omit it and Freestyle issues a free *.style.dev URL.
...
model, tools, permissions, skills) stays in `.agentica/opencode/` and
`opencode.jsonc`; `[[agents]]` declares which agents Agentica should treat as
platform-launchable and what server-side authority each one receives. Keyed by
the agent's `name`.

| Key          | Notes                                                                                           |
| ------------ | ----------------------------------------------------------------------------------------------- |
| `name`       | Agent name. In legacy OpenCode mode this usually matches `.agentica/opencode/agents/<name>.md`; declarative mode treats the manifest entry as the platform registry key. |
| `connectors` | Connector profiles the agent may call. `["slug", …]` \| `"all"` \| `"none"` (default: none).    |
| `agentica_cli` | What it may do to Agentica via the CLI/API (project-scoped iam actions). Same shape (default: none). |

```toml
[[agents]]
name       = "release-bot"
connectors = ["github"]
agentica_cli = ["project.deploy", "project.cr.open"]   # may OPEN a CR, but not merge it
```

**Grantable `agentica_cli` actions** (project-scoped only — account-level admin
actions can never be granted to an agent; run `agentica validate --scopes`):
...
agent unlisted → default-deny for Agentica grants. The grant is always
...
internals, but once a project adopts declarative agents, Agentica product UI
...
`agentica_version` is the schema version. Manifests without it are
...
it ensures `agentica_version` is the first key, so the file is
...
| Session token mint     | `[[agents]]` (per-agent connectors + agentica_cli scope)              |
...
in the **Agentica Secrets Manager** — never inline.
...
**`AGENTICA_*` is only reserved at the Secrets Manager surface**, not
at manifest parse time. The dashboard rejects creating secrets with
that prefix, but you can list `AGENTICA_FOO` in `[env]` without an
error from the parser. Don't — it'll just never have a value.

## `[sandbox]`

Sandbox base image config. **Entirely optional.** Omitting the table
(or any key) falls back to defaults.

| Key          | Default               | Notes                                                                                  |
| ------------ | --------------------- | -------------------------------------------------------------------------------------- |
| `dockerfile` | `.agentica/Dockerfile`  | Repo-relative path. Aliases: none.                                                     |
...
builder appends the Agentica runtime layer on top of your image.
...
| Key          | Default              | Notes                                                                            |
| ------------ | -------------------- | -------------------------------------------------------------------------------- |
| `config_dir` | `.agentica/opencode`   | Repo-relative dir. Same silent-fallback behavior as `[sandbox]` paths.            |
...
Do not duplicate those details in `agentica.toml`; use `[[agents]]` only for the
Agentica-side decision of which agents are launchable/authorized by the platform.
...
(`AGENTICA_TRIGGER_SCHEDULER_INTERVAL_MS`), so sub-minute precision is
best-effort.
...
- Primary header: `X-Agentica-Signature: sha256=<hmac>`. The `sha256=`
...
fly.toml-style entries inside `agentica.toml`. The platform dispatches
through a provider adapter (Freestyle today; pluggable via
`apps/api/src/deployments/providers/`) and records each deploy in the
`deployments` table.
...
1. Declare the secret name under `[env]`:
   ...
   optional = ["ANTHROPIC_API_KEY", "OPENAI_API_KEY", "WEBHOOK_SLACK_SECRET"]
   ```
2. Set the value in the Agentica Secrets Manager (dashboard).
...
- `AGENTICA_*` is reserved **at the Secrets Manager surface** — the
  CRUD endpoint rejects it. The manifest parser does not enforce
  this; declaring `AGENTICA_FOO` in `[env]` is accepted but no matching
