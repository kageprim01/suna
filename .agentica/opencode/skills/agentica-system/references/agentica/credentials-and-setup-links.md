...
secret simply appears in your session env (and in `AGENTICA_PROJECT_SECRET_NAMES`).

**Preferred — the `request_secret` tool on the `agentica-executor` MCP:**
...
```sh
agentica secrets request APOLLO_API_KEY SMARTLEAD_API_KEY     # several keys, one link
agentica secrets request APOLLO_API_KEY --scope connector     # server-side only
```
...
request**: the `add_connector` tool / `agentica executor add <slug> --provider pipedream
--app <app>` (humans: `agentica connectors add … --apply`). That commits it to
`agentica.toml` on main and syncs the catalog server-side, exactly like the
dashboard's "Add app" — it's live this session. Then mint the connect link.

**Preferred — the `connect` tool on the `agentica-executor` MCP:**
...
```sh
agentica executor connect smartlead    # the executor CLI (matches the MCP `connect` tool)
agentica connectors link smartlead     # the management CLI (durable, shareable link)
```
...
automatically (a webhook finalizes it; you don't run `agentica connectors
...
> `agentica connectors connect` (no `link`) still exists — it returns a *raw,
...
3. When they come back ("done"), **verify and continue**:
   - **Secret:** check the name is now present — `agentica secrets ls` (or look for
     it in `AGENTICA_PROJECT_SECRET_NAMES`). A fresh `runtime` value is live in the
     session env immediately (it's hot-synced; no restart needed).
   - **Connector:** check it now appears in your usable catalog —
     `agentica executor connectors` (the `connectors` MCP tool). Unconnected connectors are
...
- ❌ `agentica secrets set NAME=<value>` — you don't *have* the value, and you
  shouldn't.
...
| Goal | MCP tool | `agentica` CLI |
| --- | --- | --- |
| Ask the human for a secret value | `request_secret` | `agentica secrets request <NAME…>` |
| Get an app connected (Pipedream) | `connect` | `agentica executor connect <slug>` · `agentica connectors link <slug>` |
| Verify a secret arrived | — | `agentica secrets ls` |
| Verify a connector connected | `connectors` | `agentica executor connectors` · `agentica connectors ls` |
...
default; the `agentica executor` CLI is the same Executor core exposed for shell use.
