...
Every Agentica session runs inside its own **sandbox VM** booted from a
`[[sandbox.templates]]` image (see `agentica.toml`). It's a real Linux box: the
repo is cloned in, the toolchain declared by the image is on `$PATH`, project
secrets are injected as env vars, and the `agentica` CLI is pre-authenticated.
