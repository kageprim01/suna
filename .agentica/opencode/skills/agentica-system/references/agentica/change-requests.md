# Agentica Change Requests — full reference

A **change request** (CR) is Agentica's PR-equivalent. It proposes
merging one branch (`head_ref`) into another (`base_ref`) inside a
single Agentica project. The CR layer is **Agentica-native** — it works
on top of any git host (GitHub, GitLab, Freestyle, plain git) without
...
Agentica API against whatever backend the project's `repo_url` points
to.
...
`packages/db/src/schema/agentica.ts`):
...
to accept `agentica cr show 3` — `3` resolves to the row with
...
- `closed` is reversible: `agentica cr reopen <n>` puts it back to
...
The CLI's `agentica cr diff` falls back to no color when stdout isn't a
...
`agentica cr show <cr>` calls this automatically for open CRs and
...
The full surface is in `agentica-cli.md` alongside this doc. Summary:
...
- `--session` defaults to `$AGENTICA_SESSION_ID`, which back-fills
...
- **Merge is `Agentica <noreply@agentica.ai>`-authored.** If you want the
...
- **The `AGENTICA_*` env vars expected at `cr open` time:**
  `AGENTICA_CLI_TOKEN` (the project-scoped PAT the CLI authenticates with —
  **not** `AGENTICA_TOKEN`, which is the sandbox service key and is rejected
  by the CR routes), `AGENTICA_API_URL`, `AGENTICA_PROJECT_ID`,
  `AGENTICA_BRANCH_NAME` (or `AGENTICA_HEAD_REF`), `AGENTICA_SESSION_ID`.
  All of these are pre-injected by the session bootstrap. If you're
  running `agentica cr open` *outside* a session (e.g. on your laptop)
  you'll need to pass `--head` and `--project` explicitly, or be on a
  cwd linked via `agentica projects link`.

## See also

- `agentica-cli.md` (next to this file) — full CLI surface for `agentica
  cr`.
...
- `packages/db/src/schema/agentica.ts` — schema source (`changeRequests`
