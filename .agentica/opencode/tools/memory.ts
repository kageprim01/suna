 ...
 * reference backend — but rooted at the project's real `.agentica/memory/`
 * folder instead of a virtual `/memories` mount.
 *
 * Because every write is an ordinary file change under `.agentica/memory/`,
 * memory edits flow through the normal Agentica change-request pipeline
 * (and the `memory-reflector` agent) exactly like code.
 *
 * Paths are repo-relative and MUST live under `.agentica/memory`
 * (e.g. `.agentica/memory/overview.md`). Nothing is auto-injected: the agent
 ...
 *    `.agentica/memory-evil` cannot masquerade as the root (CVE-2026-34451);
...
const MEMORY_PREFIX = ".agentica/memory";
...
async function validatePath(memoryPath: string, projectDir: string): Promise<string> {
  ...
  // Normalize a leading "./" so both ".agentica/memory" and "./.agentica/memory" work.
  ...
  // ".agentica/memory-evil" would pass the prefix check.
...
export default tool({
  description:
    "Persistent project memory — read, write, and curate the project brain in `.agentica/memory/`. " +
    "This is the canonical way to work with memory; use it instead of the generic read/edit/write tools for anything under `.agentica/memory/`. " +
    "Memory persists across sessions and is shared with the whole team via the repo, so write durable facts here. " +
    "ALWAYS `view` `.agentica/memory` before starting a task to recover prior context, and record durable progress as you go — your context window may reset at any time.\n\n" +
    "Paths are repo-relative and MUST start with `.agentica/memory` (e.g. `.agentica/memory/overview.md`). " +
    ...
    "Always keep `.agentica/memory/MEMORY.md` (the index) in sync — one line per sub-file. " +
  ...
  args: {
    ...
    path: tool.schema
      ...
      .optional()
      .describe(
        "Repo-relative path under `.agentica/memory` (e.g. `.agentica/memory/overview.md`). Required for view, create, str_replace, insert, delete.",
      ),
