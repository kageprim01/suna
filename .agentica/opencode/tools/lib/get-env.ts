...
const LIVE_ENV_FILES = [
  process.env.AGENTICA_AGENT_ENV_FILE,
  "/dev/shm/agentica/agent-env.sh",
  "/tmp/pt-env",
  "/etc/pt-env",
 ...
 * 2. `process.env[key]`  — Docker env, manually exported (native dev without s6)
  * 3. live agent env file — `/dev/shm/agentica/agent-env.sh` or warm-claim env files
 ...
 * (agentica-master /env API) take effect immediately — no service restart needed.
...
export function getEnv(key: string): string | undefined {
  ...
  //    agentica-master writes here on every /env POST, so values update without restart.
 ...
 * AGENTICA_API_URL. The sandbox only ever holds AGENTICA_API_URL + AGENTICA_TOKEN;
 * tools build their proxy endpoint from those two. Normalizes so it works
 * whether AGENTICA_API_URL is a bare origin or already ends in /v1 or /v1/router.
 * Returns null when AGENTICA_API_URL is unset.
 */
export function getAgenticaRouterBase(service: string): string | null {
  const raw = getEnv("AGENTICA_API_URL");
  if (!raw) return null;
  const root = raw
    .replace(/\/+$/, "")
    .replace(/\/v1\/router$/, "")
