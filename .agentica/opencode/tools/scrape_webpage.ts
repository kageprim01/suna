...
export default tool({
  ...
  async execute(args, _context) {
    // Route through the Agentica router (derived from AGENTICA_API_URL) and auth with
    // AGENTICA_TOKEN; the router injects the real upstream key. Fall back to a raw
    // FIRECRAWL_API_KEY only when AGENTICA_API_URL is unset (self-host/direct).
    const apiBaseURL = getAgenticaRouterBase("firecrawl") ?? undefined;
