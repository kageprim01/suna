...
import { getEnv, getAgenticaRouterBase } from "./lib/get-env";
...
function getSerperImagesUrl(): string {
  const override = getAgenticaRouterBase("serper");
...
async function enrichImages(images: EnrichedImage[]): Promise<EnrichedImage[]> {
  const replicateBaseUrl = getAgenticaRouterBase("replicate") ?? undefined;
  // Route through the Agentica router (derived from AGENTICA_API_URL); auth with
  // AGENTICA_TOKEN. Fall back to a raw REPLICATE_API_TOKEN only when unset.
  const replicateToken = replicateBaseUrl
    ? getEnv("AGENTICA_TOKEN")
...
export default tool({
  ...
  async execute(args, _context) {
    const serperUrlOverride = getAgenticaRouterBase("serper") ?? undefined;
    // Route through the Agentica router (derived from AGENTICA_API_URL); auth with
    // AGENTICA_TOKEN. Fall back to a raw SERPER_API_KEY only when unset.
    const apiKey = serperUrlOverride
      ? getEnv("AGENTICA_TOKEN")
      : getEnv("SERPER_API_KEY")
      ? "Error: AGENTICA_TOKEN not set."
