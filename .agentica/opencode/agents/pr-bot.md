...
tokens or the `gh` CLI. Load the **`agentica-executor`** skill, then:
...
Agentica vars (\`AGENTICA_API_URL\`, \`AGENTICA_PROJECT_ID\`, \`AGENTICA_SESSION_ID\`,
`AGENTICA_TOKEN`, `AGENTICA_CLI_TOKEN`) are injected automatically.
...
```sh
WORKDIR=$(.agentica/automation/pr-bot/checkout-pr.sh)
cd "$WORKDIR"
```

Helper scripts live in `.agentica/automation/pr-bot/` (this session's repo):
...
1. Clone + `cd "$WORKDIR"`, then **materialize the project secrets into the
   ...
   ```sh
    .agentica/automation/pr-bot/write-env.sh "$WORKDIR/.env"          # repo root
    .agentica/automation/pr-bot/write-env.sh "$WORKDIR/apps/api/.env" # backend
    .agentica/automation/pr-bot/write-env.sh "$WORKDIR/apps/web/.env" # frontend
   ```
...
3. For each running port, mint a one-click URL:
    `.agentica/automation/pr-bot/preview-url.sh <port> "<label>"`
