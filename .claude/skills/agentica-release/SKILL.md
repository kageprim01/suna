...
description: "How to cut a Agentica production release — the versioning philosophy (when patch vs minor vs major) and the exact flow: derive the release title + notes from the FULL git log since the last release, run the Promote workflow, then deploy + verify prod. Load WHENEVER the user wants to release, promote, cut/ship a version, publish a release, or bump the version. The release notes ARE the public /changelog, so they must be 100% accurate to what shipped."
---
...
Pairs with **agentica-voice** (how the notes read). The version source of truth is the
...
- **notes** — a readable, grouped summary of what's in the log. Lead with user-facing
  ...
  **agentica-voice**: plain language, no hype/banned words, "open / source-available".
...
```bash
curl -fsS https://staging-api.agentica.com/v1/health | jq '{environment,version,commit}'
curl -fsS https://staging.agentica.com/api/runtime-config | grep 'staging-api.agentica.com'
gh run list --repo kageprime/suna --branch staging --limit 10
```

`qa-staging` must target `staging.agentica.com` / `staging-api.agentica.com`. A green
...
```bash
gh workflow run promote.yml --repo kageprime/suna --ref staging \
...
```bash
gh run list --repo kageprime/suna --workflow build-staging.yml --branch staging --limit 3 \
...
and Vercel deploys `prod` to `agentica.com`.

```bash
gh run list --repo kageprime/suna --workflow deploy-prod.yml --limit 1
curl -fsS https://api.agentica.com/v1/health | jq '{environment,version,commit}'
gh release view "vX.Y.Z" --repo kageprime/suna --json name,body   # title + notes present
```
...
- **Staging is not dev.** If `staging.agentica.com/api/runtime-config` references
  `dev-api.agentica.com`, stop and fix staging before releasing.
...
gh release delete vX.Y.Z --repo kageprime/suna           # only if a Release was actually cut
...
```bash
gh workflow run rollback-prod.yml --repo kageprime/suna --ref main \
...
(auto-sync + selfHeal) rolls agentica-api + agentica-gateway to those images. Then:
- **Frontend:** Vercel → agentica.com → Deployments → the `vX.Y.Z` prod deployment →
...
3. title + notes written in agentica-voice, accurate to the log.
4. `staging.agentica.com` and `staging-api.agentica.com` verified on the staging SHA.
5. `promote.yml` run with title + notes; release PR merged into `prod`.
6. deploy-prod ran, EKS green, api.agentica.com on the new version.
