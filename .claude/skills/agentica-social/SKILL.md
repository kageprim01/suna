...
Turn product context into scroll-stopping, platform-native content — and turn one piece of content into many. This skill writes the words and the scripts; it hands visual production to `agentica-image`.
...
3. **Visuals** — for carousels, quote graphics, thumbnails, or video covers, don't describe pixels here — hand the brief to [`../agentica-image/SKILL.md`](../agentica-image/SKILL.md) (which honors `brand-guidelines`).
...
- Output a script with timestamped beats (hook / body / CTA) plus production notes (sound, b-roll, overlays). Hand any cover frame or thumbnail to `agentica-image`.
...
- **agentica-image** — produces the carousels, graphics, thumbnails, and video covers
