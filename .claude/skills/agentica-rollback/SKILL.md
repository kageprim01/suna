...
agentica.com. There is no "image tag" to point at. A FE rollback is instead a
...
```
1. FE promoted to old build       → agentica.com = vX.Y.Z FE   ✅
2. merge the backend rollback PR   → push to prod
3. Vercel rebuilds FE from branch source (still the NEW code) → agentica.com = new FE ❌ CLOBBERED
```
...
at ~v0.9.72 (image exists from 0.9.69), so there is no `agentica-gateway:0.9.68`. The
...
for staging deploy + QA, then **Promote to Production** (see agentica-release).
...
- **The deploy-prod run on the backend merge goes partly red** (its version-watch
  ...
  independently. Judge success by Argo health + `api.agentica.com/v1/health`.
...
- **Verify after:** `api.agentica.com/v1/health` (version), Argo app health, and the
