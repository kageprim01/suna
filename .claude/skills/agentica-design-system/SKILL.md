...
- **Color tokens:** use the implemented tokens exposed in `@theme inline`: `background/foreground`, `card/card-foreground`, `popover/popover-foreground`, `primary/primary-foreground`, `secondary/secondary-foreground`, `muted/muted-foreground`, `accent/accent-foreground`, `destructive/destructive-foreground`, `border`, `input`, `ring`, `sidebar-*`, `chart-1`…`chart-5`, and brand accents `agentica-base`, `agentica-blue`, `agentica-yellow`, `agentica-orange`, `agentica-green`, `agentica-purple`, `agentica-red`. In components, write classes like `bg-background`, `text-foreground`, `bg-card`, `text-muted-foreground`, `border-border`, `ring-ring`, `bg-agentica-blue`. **Never** use raw Tailwind palette classes (`bg-blue-500`, `text-red-400`, `bg-green-600`, `border-amber-300`, etc.), raw values (`#fff`, `oklch(...)`, `rgb(...)`), or hardcoded theme hacks (`bg-black/10`, `bg-white/10`).
- **Accent discipline:** the app is neutral first. Use one `agentica-*` accent per surface when content needs color; use `chart-*` only for data visualization. For normal app selection and activity, prefer `primary` tint (`bg-primary/[0.05]`–`bg-primary/[0.10]`, `text-primary`, `border-l-primary`) over flat grey fills.
...
- ✅ Color/radius via `globals.css` tokens (`bg-muted`, `bg-agentica-blue`, `rounded-2xl`). ❌ Tailwind palette colors (`bg-blue-500`, `text-red-400`, `bg-green-600`), raw values (`#fff`, `oklch(…)`), `rounded-[5px]`, `rounded-md`/`rounded-xl` on a container.
...
1. Open `/design-system` (run the app, see `agentica-design-system`/run skills) and skim `src/components/ui/` before writing UI.
